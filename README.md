...
## 배포된 Dev 데이터(dev.txt)로 모델 평가 방법 

$python prediction.py --input_text=dev.txt --config_file=koelectra-base-v3_dev.json

## 평가용 prediction 코드 실행 방법
...
1. EDA: Easy Data Augmentation Techniques for Boosting Performance on Text Classification Tasks 논문을 인용하여 활용
Train dataset의 절반 label이 80문장 미만으로 파악되어
sr, ri를 augmentation한 데이터 train 76705개 dev 24895개를 생성하여 모델 학습하고
rs, rd를 augmentation한 데이터 train 54863개 dev 23445개를 생성하여 모델 학습하고
augmentation하지 않은 train 83712개의 데이터를 가지고 모델을 학습함.
(augmentation 할 때 순수하게 train data만 사용하여 aug_train 및 aug_dev를 생성하였습니다.)

2. Ensemble 기법을 활용
sr, ri를 학습한 모델 60000ckpt, 36000ckpt를 사용
rs, rd를 학습한 모델 64000ckpt, 36000ckpt를 사용
원본 데이터를 학습한 모델 32000ckpt, 40000ckpt를 사용

6개의 모델의 결과를 max voting하여 ensemble 결과를 만들어냄.

## 실험 결과

Dev set을 이용하여 학습한 결과 : 

(sr, ri 60000ckpt, 36000ckpt)
acc = 0.7043779800606849, acc = 0.6921326397919376

(rs, rd 64000ckpt, 36000ckpt)
acc = 0.7087126137841352, acc = 0.7029692241005635

(원본 데이터 32000ckpt, 40000ckpt)
acc = 0.7012353706111834, acc = 0.7064369310793238

Ensemble acc = 0.7342869527524925


## 결론

data augmentation을 어느정도 다양하게 시도해 보았는데 원본 데이터와 single model 기준으로 큰 차이를 느낄 수 없었음.
하지만 ensemble을 할 때 원본 데이터셋으로만 학습한 모델로 ensemble 할 때보다
data augmentation을 다양하게 한 모델을 섞을 때 좋은 결과를 보였다.
또한, 마지막에 가까운 checkpoint인 acc가 높은 모델들 ex(60000ckpt, 58000ckpt)
로만 조합하였을 경우 오히려 낮은 점수가 나오고 중간 ckpt들을 섞어 주었을 때 더 좋은 결과를 보여주었다.
위와 같은 실험 결과로 기본 학습 모델보다 보다 더 높은 정확도의 결과를 도출할 수 있었다.

향 후 실험 과제로는 아래의 아쉬움을 진행해보면 좋을 것 같음.
->
sr, ri, rs, rd 모두를 사용한 데이터를 평가에 사용해보지 못해 아쉬웠음.
오픈된 한글 wordnet이 퀄리티가 떨어져서 sr, ri의 성능이 아쉬움.
data augmentaion할 때 다양한 비율로 해보지 못한 것도 아쉬웠음.
batch size, learning rate 등 다양한 하이퍼파라미터를 다양하게 실험해보지 못하여 아쉬웠음.
시간제약으로 인해 다양한 학습을 돌려볼 수 없어 아쉬웠음.
더욱 다양한 ensemble 조합을 시도해보았으면 하는 아쉬움이 남음.


## 참고(사용된 코드 및 자료)

https://github.com/monologg/KoELECTRA
-> Pretrained model 및 customize 전 finetuning base code 사용

https://github.com/catSirup/KorEDA/tree/master
-> Easy Data Augmentation 한글 구현코드를 활용하여 customize함.
