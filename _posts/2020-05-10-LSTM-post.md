...
read_data() 함수를 선언해 준다.
이 함수는 데이터를 Read한 후 \t 단위로 text 데이터와 label 데이터를 구분하여 split 한 후 반환한다.
```python
...
tokenizer_tag() 함수를 선언해준다.
이 함수는 데이터를 konlpy 형태소 분석기중 Okt를 이용하여 형태소 분석을 하여 해당 데이터를 반환해준다.
```python
...
One_hot() 함수를 선언해준다.
이 함수는 label 데이터를 one-hot encoding 형태로 변환해준다.
```python
def One_hot(data):
